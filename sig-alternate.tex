...
\newpage
Although all the above should be defined by the administrator/network engineer, it would be an advantage if the \textit{keep DC policy} could be done automatically (configurable periodicity) by using the already implemented policy in the \textit{user-defined logic}.
...
For simplicity purpose, the algorithm took only into consideration the servers, but as it follows the same logic, the path which is used by the VM could also be taken into consideration.

\newpage

For analyzing the algorithms behaviour, two test were made using BF and WF.
Although they do not, by far, represent all the types of VM allocation policies, they are a good starting point for the validation of the algorithm.
